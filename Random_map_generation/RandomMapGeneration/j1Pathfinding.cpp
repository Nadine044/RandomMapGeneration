...
{
	if (App->roomsMap->Get(pos.x, pos.y) < App->roomsMap->open.size() - 1)
	{
		DungeonNode* t = App->roomsMap->open[App->roomsMap->Get(pos.x, pos.y)];
		if (App->roomsMap->CheckBoundaries(t->position))
		{
			if (!t->InvalidWalkability)
				return true;
			else
				return false;
		}
...
DungeonNode* j1PathFinding::GetTileAt(const iPoint& pos) const
{
	if (CheckBoundaries(pos))
		return App->roomsMap->open[App->roomsMap->Get(pos.x, pos.y)];
}
